...
export async function fetchBinanceSpotKlineData(
  symbol: string,
  interval: string,
  startTime: number
): Promise<any> {
  ...
  const klineData = data.map((d: any[]) => mapHttpToKlineObj(d));
  return klineData;
}

export function mapHttpToKlineObj(arr: any[]): BinanceKlineObj {
